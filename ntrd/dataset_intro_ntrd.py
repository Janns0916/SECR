...
class dataset(object):
    ...
    # 跑推荐的函数
    def padding_w2v(self,sentence,max_length,is_response=False,movies_gth=None,transformer=True,pad=0,end=2,unk=3):
        vector = []
        concept_mask = []
        dbpedia_mask = []
        reviews_mask = []
        ...
        if len(vector)>max_length:
            if transformer:
                ...
                if movie_nums != 0:
                    ...
                    if movies_gth == None:
                        pass
                        # print("movies_gth is None", movies_gth)
                    else:
                        movies_gth = movies_gth[-movie_nums:]
                else:
                    movies_gth = []
                return vector[-max_length:],max_length,concept_mask[-max_length:],dbpedia_mask[-max_length:],reviews_mask[:max_length],introduction_mask[:max_length],movies_gth
            else:
                movie_nums = sum(movie_mask[:max_length])
                movies_gth = movies_gth[:movie_nums]
                return vector[:max_length],max_length,concept_mask[:max_length],dbpedia_mask[:max_length],reviews_mask[:max_length],introduction_mask[:max_length],movies_gth
            ...
            return vector+(max_length-len(vector))*[pad],length,\
                   ...
                   introduction_mask + (max_length - len(vector))*[0],movies_gth
    ...
    def padding_context(self,contexts,pad=0,transformer=True):
        ...
        if transformer==False:
            ...
            vec,v_l,concept_mask,dbpedia_mask,reviews_mask, introduction_mask,_ = self.padding_w2v(contexts_com,self.max_c_length,transformer)
    ...
    def data_process(self,is_finetune=False):
        ...
        for line in self.data:
            ...
            response,r_length,_,_,_,_,movies_gth=self.padding_w2v(line['response'],self.max_r_length,transformer=True,is_response=True,movies_gth=line['all_movies'])

            movies_gth, movies_num = self.padding_all_movies(movies_gth, self.max_r_length)
            if False:
                mask_response,mask_r_length,_,_,_,_,_ =self.padding_w2v(self.response_delibration(line['response']),self.max_r_length)
            else:
            ...
            data_set.append([np.array(context),c_lengths,np.array(response),r_length,np.array(mask_response),mask_r_length,line['entity'],line['entities_altitude'],line['entities_altitude_attr'],
                             line['movie'], line['movie_altitude'], concept_mask,dbpedia_mask,reviews_mask,introduction_mask,line['rec'], np.array(movies_gth), movies_num])
        return data_set
    ...
    def detect_movie(self,sentence, movies):
        ...
        return token_text_com, movie_rec_trans, rev_count, intro_count, list(set(entities)), list(set(info_attr)),all_movie_selection_label,masked_movie_num
    ...
    def _context_reformulate(self,context,movies,altitude,ini_altitude,s_id,re_id):
        ...
        for message in context:
            ...
            token_text, movie_rec, temp_rev_count, temp_intro_count, intro_entity, entity_attr,all_movie_selection_label,masked_movie_num = self.detect_movie(message['text'], movies)
            ...
            if len(context_list)==0:
                ...
                context_dict={'text':token_text, 'entity': final_entity, 'entity_attr':entity_attr, 'user':message['senderWorkerId'], 'movie':movie_rec, 'movies_selection_labels': all_movie_selection_label}
                # ******************************************'entity_attr':entity_attr,
            ...
            if context_dict['user']==re_id and len(contexts)>0:
                ...
                if len(context_dict['movie']) != 0:
                    ...
                    for movie in context_dict['movie']:
                        ...
                        cases.append({'contexts': deepcopy(contexts), 'response': response, 'entity': deepcopy(entities), 'entity_attr':entity_attr,'movie': movie, 'entities_altitude': entities_altitude,'entities_altitude_attr':entities_altitude_attr, 'movie_altitude':movie_altitude,'rec': 1,'all_movies':context_dict['movies_selection_labels']})
                else:
                    ...
                    cases.append({'contexts': deepcopy(contexts), 'response': response, 'entity': deepcopy(entities), 'entity_attr':entity_attr, 'entities_altitude_attr':entities_altitude_attr, 'movie': 0,'entities_altitude': entities_altitude,'movie_altitude':movie_altitude,'rec': 0,'all_movies':context_dict['movies_selection_labels']})
...
class CRSdataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        '''
        context, c_lengths, response, r_length, mask_response, mask_r_length, entity,entities_altitude, entities_altitude_attr, movie, movie_altitude, concept_mask, dbpedia_mask, reviews_mask, introduction_mask, rec, movies_gth, movies_num = self.data[index]
        ...
        return context, c_lengths, response, r_length, mask_response, mask_r_length, entity_vec, entity_vector, entities_altitude_vec, entities_altitude_attr_vec, movie, movie_altitude,np.array(concept_mask), np.array(dbpedia_mask), np.array(reviews_mask), np.array(introduction_mask), concept_vec, db_vec, rec, movies_gth, movies_num
